...
//串口收发缓存
u8 UART1RXCache[UART1RXLen]; //温湿度大气压传感器数据串口缓�?
u8 UART1TXCache[UART1TXLen]; //温湿度大气压传感器指令串口缓�?
u8 UART2RXCache[UART2RXLen]; //九轴传感器数据串口缓�?
...
void SensorTaskF(void const * argument)
{
  /* USER CODE BEGIN SensorTaskF */
	WT931Point(UART2RXCache); //九轴传感器读取指针定�?
	...
	for (;;)
	{
		WaterDetect = HAL_GPIO_ReadPin(GPIOD, GPIO_PIN_10); //�?测是否漏�?
		if (WT931Finish) //如果传感器读取完�?
		{
			while (WT931IO) //等待传感器读�?
			{
				osDelay(1);
			}
			WT931IO = 1; //正在读写传感器缓�?
			if (WT931Len == 44) //�?测传感器数据长度是否正确
			{
				WT931Take(AccelerationCache, RotSpeedCache, EulerAngleCache,
						MagnetisCache);
			}
			WT931Finish = 0; //已完成传感器数据取出
			HAL_UART_Receive_DMA(&huart2, UART2RXCache, UART2RXLen); //接收上位机指令数�?
			__HAL_UART_ENABLE_IT(&huart2, UART_IT_IDLE);
			WT931IO = 0; //传感器缓存读写完�?
		}

		if (GY39Finish) //如果传感器读取完�?
		{
			while (GY39IO) //等待传感器读�?
			{
				osDelay(1);
			...
			HAL_UART_Receive_DMA(&huart1, UART1RXCache, UART1RXLen); //接收九轴传感器数�?
