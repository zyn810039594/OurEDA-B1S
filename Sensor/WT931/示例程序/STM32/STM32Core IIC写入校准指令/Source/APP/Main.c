...
{  
  unsigned char chrTemp[30];
	unsigned char i=0;
	unsigned char cmd1[2] = {0X00,0X01};//可以参考一下使用说明书对寄存器写入的值
	//unsigned char cmd0[2] = {0X00,0X00};//可以参考一下使用说明书对寄存器写入的值
	...
	while (1)
	{
		...
		delay_ms(1000);
		IICreadBytes(0x50, AX, 24,&chrTemp[0]);//读取从AX寄存器之后的24个字节长度的数据，也就是XYZ的加速度角速度磁场和角度
		a[0] = (float)CharToShort(&chrTemp[0])/32768*16;//高低字节合并成一个16位长度的数据进行计算
